...
void rpe::Value::initGdd(std::string typeStr, bool isEnum, uint32_t count, bool forceStr) {
    ...
    if (epicsType_ == aitEnumInvalid) {
        log_->info("Detected unknown type for %s typeStr=%s. It will be mapped to a string.",
                   epicsName_.c_str(),
                   typeStr.c_str());
        epicsType_ = aitEnumString;
    }
    ...
    // Vector
    if (count != 0) {
        log_->info("Create vector GDD for %s epicsType_=%" PRIu32 ", size=%" PRIu32,
                   epicsName_.c_str(),
                   epicsType_,
                   count);
...
bool rpe::Value::valueSet() {
    return true;
}

// Value lock held when this is called
bool rpe::Value::valueGet() {
    return true;
}
...
gddAppFuncTableStatus rpe::Value::readEnums(gdd& value) {
    ...
    if (epicsType_ == aitEnumEnum16) {
        ...
        for (i = 0; i < nstr; i++) strncpy(str[i].fixed_string, enums_[i].c_str(), sizeof(str[i].fixed_string));
