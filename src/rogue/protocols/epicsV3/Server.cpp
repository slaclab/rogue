...
void rpe::Server::start() {
    ...
    if (workCnt_ > 0) {
        workersEn_ = true;
        for (x = 0; x < workCnt_; x++) workers_[x] = new std::thread(boost::bind(&rpe::Server::runWorker, this));
    }
    running_ = true;
}
...
void rpe::Server::stop() {
    ...
    if (running_) {
        ...
        for (x = 0; x < workCnt_; x++) workQueue_.push(std::shared_ptr<rogue::protocols::epicsV3::Work>());
...
pvCreateReturn rpe::Server::createPV(const casCtx& ctx, const char* pvName) {
    ...
    if ((it = values_.find(pvName)) == values_.end()) return S_casApp_pvNotFound;
