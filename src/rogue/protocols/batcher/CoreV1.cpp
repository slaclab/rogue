...
bool rpb::CoreV1::processFrame(ris::FramePtr frame) {
    ...
    switch ((temp >> 4) & 0xF) {
        case 0:
            headerSize_ = 2;
            break;  // If WIDTH=0x0 (TYPE = 16-bit AXI stream),  then appended header is 2 bytes.
        case 1:
            headerSize_ = 4;
            break;  // If WIDTH=0x1 (TYPE = 32-bit AXI stream),  then appended header is 4 bytes.
        case 2:
            headerSize_ = 8;
            break;  // If WIDTH=0x2 (TYPE = 64-bit AXI stream),  then appended header is 8 bytes.
        case 3:
            headerSize_ = 16;
            break;  // If WIDTH=0x3 (TYPE = 128-bit AXI stream), then appended header is 16 bytes.
        case 4:
            headerSize_ = 32;
            break;  // If WIDTH=0x4 (TYPE = 256-bit AXI stream), then appended header is 32 bytes.
        case 5:
            headerSize_ = 64;
            break;  // If WIDTH=0x5 (TYPE = 512-bit AXI stream), then appended header is 64 bytes.
        default:
            log_->error("Invalid AXIS Type Detected. Got %" PRIu8, ((temp >> 4) & 0xF));
            return false;
    }
    ...
    if (rem < (headerSize_ + tailSize_)) {
        log_->error("Not enough space (%" PRIu32 ") for tail (%" PRIu32 ") + header (%" PRIu32 ")",
                    rem,
                    headerSize_,
                    tailSize_);
        reset();
