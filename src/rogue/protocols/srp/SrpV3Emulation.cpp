...
void rps::SrpV3Emulation::runThread() {
    ...
    while (threadEn_) {
        ...
        // Catch exceptions (e.g. OOM from allocatePage) to prevent std::terminate
        try {
            processFrame(frame);
...
uint8_t* rps::SrpV3Emulation::allocatePage(uint64_t addr4k) {
    ...
    // Fill with random data to emulate uninitialized hardware memory (memcpy avoids strict-aliasing UB)
    ...
    // Release ownership only after successful insert (throwing emplace won't leak)
