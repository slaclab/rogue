...
rpx::JtagDriver::Header
rpx::JtagDriver::newXid()
{
   if (XID_ANY == ++xid_)
      ++xid_;
...
rpx::JtagDriver::Xid
rpx::JtagDriver::getXid(Header x)
{
   return (x >> 20) & 0xff;
}

uint32_t
rpx::JtagDriver::getCmd(Header x)
{
   return x & CMD_MASK;
}

unsigned
rpx::JtagDriver::getErr(Header x)
{
   if (getCmd(x) != CMD_E)
      return 0;
...
unsigned long
rpx::JtagDriver::getLen(Header x)
{
   if (getCmd(x) != CMD_S)
      throw(rogue::GeneralError::create("JtagDriver::getLen()", "Cannot extract length from non-shift command header"));

   return ((x & LEN_MASK) >> LEN_SHIFT) + 1;
}

const char *
rpx::JtagDriver::getMsg(unsigned e)
{
   switch (e)
   {
      case 0:
...
rpx::JtagDriver::Header
rpx::JtagDriver::mkQuery()
{
   return PVERS | CMD_Q | XID_ANY;
}

rpx::JtagDriver::Header
rpx::JtagDriver::mkShift(unsigned len)
{
   len = len - 1;
...
unsigned
rpx::JtagDriver::wordSize(Header reply)
{
   return (reply & 0x0000000f) + 1;
}

unsigned
rpx::JtagDriver::memDepth(Header reply)
{
   return (reply >> 4) & 0xffff;
}

uint32_t
rpx::JtagDriver::cvtPerNs(Header reply)
{
...
unsigned
rpx::JtagDriver::getWordSize()
{
   return wordSize_;
}

unsigned
rpx::JtagDriver::getMemDepth()
{
   return memDepth_;
}

rpx::JtagDriver::Header
rpx::JtagDriver::getHdr(uint8_t *buf)
{
   Header hdr;
...
rpx::JtagDriver::query()
{
   ...
   if (wordSize_ < sizeof(hdr))
   {
      ...
      throw(rogue::GeneralError::create("JtagDriver::query()", "Received invalid word size. Please ensure the board is powered up.\n"));
   }
...
uint32_t
rpx::JtagDriver::getPeriodNs()
{
   return periodNs_;
}

uint32_t
rpx::JtagDriver::setPeriodNs(uint32_t requestedPeriod)
{
