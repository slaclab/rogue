...
bool
rpxx::JtagDriverLoopBack::rdl(char *buf, size_t bufsz)
{
...
void
rpxx::JtagDriverLoopBack::checkTDI(unsigned long val)
{
	if ( ! tdoOnly_ ) {
		check(val, "TDI : %li");
	}
...
void
rpxx::JtagDriverLoopBack::checkTMS(unsigned long val)
{
	if ( ! tdoOnly_ ) {
		check(val, "TMS : %li");
	}
...
void
rpxx::JtagDriverLoopBack::checkLEN(unsigned long val)
{
	if ( ! tdoOnly_ ) {
...
void
rpxx::JtagDriverLoopBack::setValLE(unsigned long val, uint8_t *buf, unsigned wsz)
{
	while ( wsz > 0 ) {
		*buf++ = val;
		val >>= 8;
