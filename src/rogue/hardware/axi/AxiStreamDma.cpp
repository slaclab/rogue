...
rha::AxiStreamDma::AxiStreamDma(std::string path, uint32_t dest, bool ssiEnable) {
    ...
    if (dmaCheckVersion(fd_) < 0)
        throw(rogue::GeneralError("AxiStreamDma::AxiStreamDma",
    ...
    if (rawBuff_ == NULL) {
        throw(
            rogue::GeneralError("AxiStreamDma::AxiStreamDma",
                                "Failed to map dma buffers. Increase vm map limit: sysctl -w vm.max_map_count=262144"));
    }
    // else if ( bCount_ >= 1000 ) retThold_ = 50;
    else if (bCount_ >= 1000)
        retThold_ = 80;
