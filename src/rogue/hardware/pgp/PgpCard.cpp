...
rhp::PgpCard::PgpCard(std::string path, uint32_t lane, uint32_t vc) {
    ...
    if (dmaCheckVersion(fd_) < 0)
        throw(rogue::GeneralError("PgpCard::PgpCard",
    ...
    if (dmaSetMaskBytes(fd_, mask) < 0) {
        ::close(fd_);
        throw(rogue::GeneralError::create("PgpCard::PgpCard",
                                          "Failed to acquire destination %" PRIu32 " on device %s",
                                          (lane * 4) + vc,
                                          path.c_str()));
    }
        ...
        while (alloc < size) {
            ...
            do {
                ...
                if (select(fd_ + 1, NULL, &fds, NULL, &tout) <= 0) {
                    log_->critical("PgpCard::acceptReq: Timeout waiting for outbound buffer after %" PRIuLEAST32
    ...
    for (it = frame->beginBuffer(); it != frame->endBuffer(); ++it) {
        ...
        // Write to pgp with (*it)er copy in driver
            ...
            do {
                ...
                if (select(fd_ + 1, NULL, &fds, NULL, &tout) <= 0) {
                    log_->critical("PgpCard::acceptFrame: Timeout waiting for outbound write after %" PRIuLEAST32
                                   ".%" PRIuLEAST32 " seconds! May be caused by outbound back pressure.",
                                   timeout_.tv_sec,
                                   timeout_.tv_usec);
                    res = 0;
...
void rhp::PgpCard::runThread(std::weak_ptr<int> lockPtr) {
    ...
    while (threadEn_) {
        ...
        if (select(fd_ + 1, &fds, NULL, NULL, &tout) > 0) {
            ...
            // Return of -1 is bad
            if (res < 0) throw(rogue::GeneralError("PgpCard::runThread", "DMA Interface Failure!"));
...
void rhp::PgpCard::setup_python() {
    ...
    bp::class_<rhp::PgpCard, rhp::PgpCardPtr, bp::bases<ris::Master, ris::Slave>, boost::noncopyable>(
        "PgpCard",
        bp::init<std::string, uint32_t, uint32_t>())
