...
#ifndef NO_PYTHON
    bp::class_<ruf::LegacyStreamWriter, ruf::LegacyStreamWriterPtr, bp::bases<ruf::StreamWriter>, boost::noncopyable>(
        "LegacyStreamWriter",
        bp::init<>())
...
void ruf::LegacyStreamWriter::writeFile(uint8_t channel, std::shared_ptr<rogue::interfaces::stream::Frame> frame) {
    ...
    if (fd_ >= 0) {
        ...
        if (channel == RawData) {
            if ((size % 4) != 0)
                throw(rogue::GeneralError::create("LegacyStreamWriter::writeFile",
                                                  "FrameSize %" PRIu32 " is not 32-bit aligned.",
                                                  size));
            size = size / 4;
        }
