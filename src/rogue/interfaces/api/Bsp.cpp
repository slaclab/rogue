...
void ria::Bsp::addVarListener(void(*func)(std::string,std::string), void(*done)()) {
   if ( this->_isRoot ) this->_obj.attr("_addVarListenerCpp")(func, done);
   else throw(rogue::GeneralError::create("Bsp::addVarListener", "Attempt to execute addVarListener on non Root node %s", this->_name.c_str()));
}
...
std::string ria::Bsp::getAttribute(std::string attribute) {
   try {
      ...
      throw(rogue::GeneralError::create("Bsp::getAttribute", "Invalid attribute %s for node %s", attribute.c_str(), this->_name.c_str()));
   }
}
...
rogue::interfaces::api::Bsp ria::Bsp::operator [](std::string name) {
   try {
      ...
      throw(rogue::GeneralError::create("Bsp::[]", "Invalid child node %s for node %s", name.c_str(), this->_name.c_str()));
   }
}
...
std::shared_ptr<rogue::interfaces::api::Bsp> ria::Bsp::getNode(std::string name) {
   try {
      ...
      throw(rogue::GeneralError::create("Bsp::node", "Invalid child node %s for node %s", name.c_str(), this->_name.c_str()));
   }
}
...
void ria::Bsp::set(std::string value) {
   try {
      ...
      throw(rogue::GeneralError::create("Bsp::set", "Error setting value %s on node %s", value.c_str(), this->_name.c_str()));
   }
}
