...
ris::FramePtr ris::Master::reqFrame(uint32_t size, bool zeroCopyEn) {
    ...
    if (slaves_.size() == 0)
        return (defSlave_->acceptReq(size, zeroCopyEn));
    else
...
void ris::Master::sendFrame(FramePtr frame) {
    ...
    for (rit = slaves.rbegin(); rit != slaves.rend(); ++rit) (*rit)->acceptFrame(frame);
}
...
void ris::Master::equalsPy(boost::python::object p) {
    ...
    if (rMst == NULL || rSlv == NULL || lSlv == NULL)
        throw(rogue::GeneralError::create("stream::Master::equalsPy",
                                          "Attempt to use == with an incompatible stream slave"));
...
bp::object ris::Master::rshiftPy(bp::object p) {
    ...
    if (slv != NULL)
        ...
        throw(rogue::GeneralError::create("stream::Master::rshiftPy",
                                          "Attempt to use >> with incompatible stream slave"));
