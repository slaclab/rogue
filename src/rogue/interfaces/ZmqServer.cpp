...
void rogue::interfaces::ZmqServer::setup_python() {
    ...
    bp::class_<rogue::interfaces::ZmqServerWrap, rogue::interfaces::ZmqServerWrapPtr, boost::noncopyable>(
        "ZmqServer",
    ...
    if (!res) {
        if (port == 0)
            throw(rogue::GeneralError::create("ZmqServer::ZmqServer",
                                              "Failed to auto bind server on interface %s.",
                                              this->addr_.c_str()));
        else
            throw(rogue::GeneralError::create("ZmqServer::ZmqServer",
                                              "Failed to bind server to port %" PRIu16
                                              " on interface %s. Another process may be using this port.",
                                              port + 1,
                                              this->addr_.c_str()));
    }
...
bool rogue::interfaces::ZmqServer::tryConnect() {
    ...
    log_->debug("Trying to serve on ports %" PRIu16 ":%" PRIu16 ":%" PRIu16,
                this->basePort_,
                this->basePort_ + 1,
                this->basePort_ + 2);
...
rogue::interfaces::ZmqServerWrap::ZmqServerWrap(std::string addr, uint16_t port)
    : rogue::interfaces::ZmqServer(addr, port) {}
