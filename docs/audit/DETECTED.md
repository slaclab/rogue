...
| STREAM-015 | src/rogue/interfaces/stream/TcpCore.cpp | 183 | threading | medium | GilRelease inside TcpCore::acceptFrame (cpp worker callback) — GIL not held by this thread; PyEval_SaveThread on an unowned GIL is a no-op on CPython but implementation-defined behavior | detected-only | Static analysis only | GIL-048 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| STREAM-016 | src/rogue/interfaces/stream/Fifo.cpp | 116 | threading | medium | GilRelease inside Fifo::acceptFrame (cpp worker callback) — GIL not held by this thread; wrong-direction usage per D-06; same class as STREAM-015 | detected-only | Static analysis only | GIL-053 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| STREAM-017 | src/rogue/interfaces/stream/Slave.cpp | 79 | threading | medium | GilRelease inside stream Slave::acceptFrame (cpp worker) — GIL not held by this cpp worker thread; wrong-direction per D-06 | detected-only | Static analysis only | GIL-054 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| MEM-001 | src/rogue/interfaces/memory/Slave.cpp | 55-59 | threading | high | classMtx_.lock() / .unlock() used raw without RAII; if classIdx_ increment or store throws (unlikely on x86, but UB-possible), the lock leaks and every subsequent Slave construction deadlocks | detected-only | Inject exception between lines 55-59 under TSAN stress; static analysis via -fsanitize=thread should flag the non-RAII pattern at init time | CONCERNS.md §Logging Mutex Manual Lock Pattern (analogous) | stress_raw_lock_unlock.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| MEM-002 | src/rogue/interfaces/memory/Transaction.cpp | 94-98 | threading | high | classMtx_.lock() / .unlock() used raw without RAII in Transaction::Transaction(); same deadlock-on-exception risk as MEM-001; any future reorder inside the ctor leaves the lock held | detected-only | Run TSAN + construction stress under OOM pressure; static analysis flags the bare lock/unlock pair | CONCERNS.md §Logging Mutex Manual Lock Pattern | stress_raw_lock_unlock.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| MEM-003 | src/rogue/interfaces/memory/TcpClient.cpp | 117-139 | threading | medium | threadEn_ is a plain bool written by stop() (from Python/main thread) and read by runThread() (from std::thread worker) without any lock or std::atomic qualification | detected-only | TSan will flag this; run the TcpClient integration test under TSan | — | stress_atomic_conversions.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| MEM-004 | src/rogue/interfaces/memory/TcpServer.cpp | 101-159 | threading | medium | Same non-atomic threadEn_ pattern as MEM-003 in TcpServer; stop() writes threadEn_=false from the calling thread while runThread() reads it in while(threadEn_) | detected-only | TSan will flag this under stop/join concurrent with recv loop | — | stress_atomic_conversions.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| MEM-011 | src/rogue/interfaces/memory/Transaction.cpp | 156-178 | threading | medium | Transaction::done() calls parentTran->subTranMap_.erase(id_) while holding child lock but NOT parent lock; two sibling sub-transactions completing concurrently race on subTranMap_ | detected-only | Create a transaction with two sub-transactions and complete them from two parallel threads simultaneously; TSan will flag the concurrent subTranMap_.erase() | — | stress_memory_sub_transaction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| MEM-015 | src/rogue/interfaces/memory/TransactionLock.cpp | 40-53 | threading | low | TransactionLock::TransactionLock() traverses the parentTransaction_ chain while holding GilRelease but without any lock on either the child or parent transactions; latent threading assumption | detected-only | Construct a sub-transaction chain and lock from two threads simultaneously; TSan may flag the unlocked read of isSubTransaction_ | — | stress_memory_sub_transaction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PROTO-RSSI-002 | src/rogue/protocols/rssi/Controller.cpp | 125-131, 202-219 | threading | medium | state_ read lock-free in transportRx() at lines 202-219 while runThread() writes it under stMtx_; locTryPeriod_ written by setLocTryPeriod() without synchronization (any thread) and read by runThread — torn read possible on struct timeval | detected-only | TSan would flag the bare state_ read in transportRx vs write in runThread; locTryPeriod_ set vs runThread read | CONCERNS.md §Thread-Safety-Concerns | stress_rssi_state_races.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-RSSI-003 | src/rogue/protocols/rssi/Controller.cpp | 391-412 | threading | medium | applicationRx() busy-waits on txListCount_ >= curMaxBuffers_ with bare usleep(10) loop; txListCount_ is plain uint8_t read in applicationRx without holding txMtx_ — lock-free read of non-atomic field updated under mutex | detected-only | TSan race between transportRx ACK processing (txMtx_ held) and applicationRx txListCount_ read | CONCERNS.md §Thread-Safety-Concerns | stress_rssi_state_races.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-RSSI-004 | src/rogue/protocols/rssi/Controller.cpp | 415-418 | threading | low | getOpen() reads state_ lock-free from Python thread context; state_ is uint32_t written by runThread without mutex; formally UB and TSan-visible | detected-only | TSan race between getOpen() call and runThread state transitions | CONCERNS.md §Thread-Safety-Concerns | stress_rssi_state_races.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PROTO-RSSI-008 | src/rogue/protocols/rssi/Controller.cpp | 198 | threading | medium | GilRelease inside cpp-worker transportRx — GIL not held by this thread; PyEval_SaveThread on an unowned GIL is a no-op on CPython but implementation-defined | detected-only | Static analysis only | GIL-004 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-RSSI-009 | src/rogue/protocols/rssi/Controller.cpp | 341 | threading | medium | GilRelease inside cpp-worker applicationTx — GIL not held; wrong-direction per D-06 | detected-only | Static analysis only | GIL-005 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-RSSI-010 | src/rogue/protocols/rssi/Controller.cpp | 371 | threading | medium | GilRelease inside cpp-worker applicationRx — GIL not held; wrong-direction per D-06 | detected-only | Static analysis only | GIL-006 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PROTO-UDP-005 | src/rogue/protocols/udp/Client.cpp | 147 | threading | medium | GilRelease inside UDP Client acceptFrame (cpp worker callback) — wrong-direction per D-06; GIL not held by this thread | detected-only | Static analysis only | GIL-026 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-UDP-006 | src/rogue/protocols/udp/Server.cpp | 136 | threading | medium | GilRelease inside UDP Server acceptFrame (cpp worker callback) — wrong-direction per D-06; GIL not held by this thread | detected-only | Static analysis only | GIL-027 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PROTO-PACK-002 | src/rogue/protocols/packetizer/ControllerV2.cpp | 242 | logic | low | ControllerV2::transportRx() calls tranFrame_[tmpDest]->setError(0x80) after tranFrame_[tmpDest].reset() on the EOF path; after reset() the shared_ptr is null — use-after-free/null-deref crash | detected-only | Any RSSI+packetizer session with an SSI EOF error on the last packet of a transfer will crash the process | — | stress_shared_ptr_null_deref.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-PACK-003 | src/rogue/protocols/packetizer/ControllerV1.cpp | 174 | logic | low | ControllerV1::transportRx() same pattern: tranFrame_[tmpDest]->setError(0x80) after tranFrame_[0].reset(); double bug: index mismatch (tmpDest vs 0) and use-after-reset; will crash if enSsi_ is set and an SSI error arrives at EOF | detected-only | Any ControllerV1 SSI-enabled session receiving an error on last packet of a transfer | — | stress_shared_ptr_null_deref.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PROTO-PACK-005 | src/rogue/protocols/packetizer/ControllerV1.cpp | 67 | threading | medium | GilRelease inside cpp-worker transportRx — GIL not held by this thread; wrong-direction per D-06 | detected-only | Static analysis only | GIL-008 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-PACK-006 | src/rogue/protocols/packetizer/ControllerV1.cpp | 199 | threading | medium | GilRelease inside cpp-worker applicationRx — GIL not held; wrong-direction per D-06 | detected-only | Static analysis only | GIL-009 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-PACK-007 | src/rogue/protocols/packetizer/ControllerV2.cpp | 91 | threading | medium | GilRelease inside cpp-worker transportRx — GIL not held; wrong-direction per D-06 | detected-only | Static analysis only | GIL-011 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-PACK-008 | src/rogue/protocols/packetizer/ControllerV2.cpp | 272 | threading | medium | GilRelease inside cpp-worker applicationRx — GIL not held; wrong-direction per D-06 | detected-only | Static analysis only | GIL-012 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PROTO-SRP-004 | src/rogue/protocols/srp/SrpV0.cpp | 188 | threading | medium | GilRelease inside acceptFrame (cpp worker) — wrong-direction per D-06; GIL not held by this thread | detected-only | Static analysis only | GIL-022 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-SRP-005 | src/rogue/protocols/srp/SrpV3.cpp | 212 | threading | medium | GilRelease inside acceptFrame (cpp worker) — wrong-direction per D-06; GIL not held by this thread | detected-only | Static analysis only | GIL-024 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-SRP-006 | src/rogue/protocols/srp/SrpV3Emulation.cpp | 214 | threading | medium | GilRelease inside processFrame called from runThread (cpp worker) — wrong-direction per D-06 | detected-only | Static analysis only | GIL-025 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PROTO-BATCH-002 | src/rogue/protocols/batcher/CombinerV1.cpp | 108 | threading | medium | GilRelease inside acceptFrame called from cpp worker — wrong-direction per D-06; GIL not held by this thread | detected-only | Static analysis only | GIL-013 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-BATCH-003 | src/rogue/protocols/batcher/CombinerV2.cpp | 94 | threading | medium | GilRelease inside acceptFrame called from cpp worker — wrong-direction per D-06; GIL not held | detected-only | Static analysis only | GIL-015 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-BATCH-004 | src/rogue/protocols/batcher/SplitterV1.cpp | 74 | threading | medium | GilRelease inside acceptFrame called from cpp worker — wrong-direction per D-06; GIL not held | detected-only | Static analysis only | GIL-017 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-BATCH-005 | src/rogue/protocols/batcher/SplitterV2.cpp | 74 | threading | medium | GilRelease inside acceptFrame called from cpp worker — wrong-direction per D-06; GIL not held | detected-only | Static analysis only | GIL-018 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-BATCH-006 | src/rogue/protocols/batcher/InverterV1.cpp | 73 | threading | medium | GilRelease inside acceptFrame called from cpp worker — wrong-direction per D-06; GIL not held | detected-only | Static analysis only | GIL-019 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| PROTO-BATCH-007 | src/rogue/protocols/batcher/InverterV2.cpp | 73 | threading | medium | GilRelease inside acceptFrame called from cpp worker — wrong-direction per D-06; GIL not held | detected-only | Static analysis only | GIL-020 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| HW-CORE-001 | src/rogue/Logging.cpp | 109-112 | threading | high | levelMtx_.lock() called in Logging::Logging ctor at line 109; exception thrown by loggers_.push_back or updateLevelLocked before levelMtx_.unlock() at line 112 would exit without releasing levelMtx_, permanently wedging any subsequent logger creation | detected-only | Throw std::bad_alloc from loggers_.push_back with address-space pressure before levelMtx_.unlock(); subsequent Logging::create call will deadlock | CONCERNS.md §Known-Bugs §Logging-Mutex-Manual-Lock-Unlock | stress_raw_lock_unlock.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| HW-CORE-002 | src/rogue/Logging.cpp | 120-127, 146-149, 155-163, 167-176, 181-183, 188-190 | threading | high | Six additional raw levelMtx_.lock() / levelMtx_.unlock() pairs in ~Logging, setLevel, setFilter, setForwardPython, forwardPython, setEmitStdout, emitStdout; none are exception-safe; new rogue::LogFilter inside setFilter (line 157) is the most concrete heap-allocation danger | detected-only | Inject allocation failure via LD_PRELOAD while calling Logging::setFilter under thread contention; deadlock on next setLevel | CONCERNS.md §Known-Bugs §Logging-Mutex-Manual-Lock-Unlock | stress_raw_lock_unlock.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| HW-CORE-018 | src/rogue/utilities/fileio/StreamReader.cpp | 180-231 | threading | medium | StreamReader::runThread accesses fd_ (read at line 180, written in intClose via line 144) without holding mtx_; close() acquires mtx_ and calls intClose() — data race on fd_ between thread loop and concurrent close() | detected-only | Call close() from the main thread while runThread is active reading the file; TSan should flag the unsynchronized fd_ access | — | stress_fileio_fd_race.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| HW-CORE-019 | src/rogue/utilities/fileio/StreamWriterChannel.cpp | 87-88 | threading | low | StreamWriterChannel::getFrameCount() reads frameCount_ without any lock (a data race with concurrent acceptFrame calls that increment under mtx_) | detected-only | Concurrent acceptFrame + getFrameCount calls; TSan will flag the unlocked read in getFrameCount | — | stress_stream_counters.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| HW-CORE-021 | src/rogue/utilities/Prbs.cpp | 343 | threading | medium | GilRelease inside Prbs::genFrame called from runThread (cpp worker) — GIL not held by this thread; wrong-direction per D-06 | detected-only | Static analysis only | GIL-076 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| HW-CORE-022 | src/rogue/utilities/Prbs.cpp | 423 | threading | medium | GilRelease inside Prbs::acceptFrame (cpp worker callback) — wrong-direction per D-06; GIL not held | detected-only | Static analysis only | GIL-077 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| HW-CORE-023 | src/rogue/utilities/StreamUnZip.cpp | 62 | threading | medium | GilRelease inside StreamUnZip::acceptFrame (cpp worker callback) — wrong-direction per D-06; GIL not held | detected-only | Static analysis only | GIL-078 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| HW-CORE-024 | src/rogue/utilities/StreamZip.cpp | 63 | threading | medium | GilRelease inside StreamZip::acceptFrame (cpp worker callback) — wrong-direction per D-06; GIL not held | detected-only | Static analysis only | GIL-079 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| HW-CORE-025 | src/rogue/utilities/fileio/StreamWriterChannel.cpp | 75 | threading | medium | GilRelease inside StreamWriterChannel::acceptFrame (cpp worker callback) — wrong-direction per D-06; GIL not held | detected-only | Static analysis only | GIL-084 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| HW-CORE-026 | src/rogue/utilities/fileio/StreamWriter.cpp | 316 | threading | medium | GilRelease inside StreamWriter::writeFile called from cpp worker via acceptFrame chain — wrong-direction per D-06 | detected-only | Static analysis only | GIL-096 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
| HW-CORE-027 | src/rogue/hardware/axi/AxiStreamDma.cpp | 339 | threading | medium | GilRelease inside AxiStreamDma::acceptFrame (cpp worker callback) — wrong-direction per D-06; GIL not held by this thread | detected-only | Static analysis only | GIL-102 | stress_gil_wrong_direction.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PY-003 | python/pyrogue/interfaces/_Virtual.py | 313-316 | threading | medium | VirtualNode._doUpdate iterates self._functions list without a lock; concurrent _addListener/_delListener mutations can cause RuntimeError: list changed size during iteration | detected-only | Add listener from one thread while VirtualClient's update thread fires _doUpdate; stress with 10k update/add cycles | — | stress_python_update_worker.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PY-013 | python/pyrogue/interfaces/_ZmqServer.py | 107-115 | threading | medium | ZmqServer._updateList is a plain dict mutated by _varUpdate (Root update-worker thread) and read/cleared by _varDone (same thread — safe); but _doRequest/_doString are called from C++ ZMQ REP thread context; dict can be mutated from two threads without a lock | detected-only | Send a ZMQ REQ while a batch of variable updates is flushing; TSan would flag dict read/write race | — | stress_python_update_worker.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
...
| PY-026 | python/pyrogue/pydm/data_plugins/rogue_plugin.py | 135 | threading | medium | RogueConnection._updateVariable is registered as a addListener callback on a VirtualNode; _updateVariable calls self.new_value_signal[...].emit(...) directly from the Root update-worker thread, not the Qt main thread; default connection type means signal emission crosses into the Qt event loop without explicit queuing | detected-only | Connect a PyDM label to a fast-updating Rogue variable and stress the update thread; observe occasional Qt crash or corrupted display | — | stress_python_update_worker.py; detected-only (Phase 4 D-05 variant per D-24 Class B carve-out): ci-tsan SIGABRT during test_stress_udp_threaden blocked coverage this milestone; see `04-02-MIDPHASE-RERUN.md` §0. |
