...
Key Functions
-------------
* Sub-Devices, Variables and Commands are added using the :py:pyrogue:`add() <pyrogue.Device.add>` method
* A special device-specific :py:pyrogue:`addRemoteVariables <pyrogue.Device.addRemoteVariables>` method allows the creation of RemoteVariable arrays
* Hide or Unhide all variables with :py:pyrogue:`hideVariables() <pyrogue.Device.hideVariables>`
* Optional override behavior for when a system-wide hardreset is generated - :py:pyrogue:`hardReset() <pyrogue.Device.hardReset>`
* Optional override for soft resets - :py:pyrogue:`softReset() <pyrogue.Device.softReset>`
* Optional override for countReset - :py:pyrogue:`countReset() <pyrogue.Device.countReset>`
...
* In a bulk config the shadow values in each variable are written by issuing the :py:pyrogue:`setDisp() <pyrogue.Variable.setDisp>`
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
* Write Transaction - `writeBlocks() <pyrogue.Device.writeBlocks>`
* Verify Transaction - `verifyBlocks() <pyrogue.Device.verifyBlocks>`
* Read Transation - `readBlocks() <pyrogue.Device.readBlocks>`
* Wait on the result of multiple transations - `checkBlocks() <pyrogue.Device.checkBlocks>`
