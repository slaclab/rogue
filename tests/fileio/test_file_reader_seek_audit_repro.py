...
def test_filereader_uses_seek_cur():
    """_FileReader batch-mode seek lacks os.SEEK_CUR; defaults to SEEK_SET."""
    src = inspect.getsource(file_reader_mod.FileReader.records)

    assert "SEEK_CUR" in src, (
        "_FileReader.records() batch-mode seek(bHead.width-8) is "
