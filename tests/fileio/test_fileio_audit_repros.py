#!/usr/bin/env python3
#-----------------------------------------------------------------------------
# Title      : StreamReader / StreamWriter raw-resource audit repros
#-----------------------------------------------------------------------------
...
#   UTIL-004 -- StreamReader::open() uses raw ``new std::thread``
#   UTIL-006 -- StreamWriter::setBufferSize() uses raw malloc instead of
#               std::vector
#   UTIL-010 -- StreamReader::runThread reads frame size as uint32_t and
...
def test_streamreader_uses_unique_ptr_thread_util_004():
    # UTIL-004: StreamReader::open() at
    ...
    assert "new std::thread" not in src, (
        "UTIL-004: StreamReader::open uses raw new std::thread at line ~95; "
        "should be std::unique_ptr<std::thread>"
    )


def test_streamwriter_buffer_uses_vector_util_006():
    # UTIL-006: StreamWriter::setBufferSize() at
    ...
    assert "malloc(" not in src, (
        "UTIL-006: StreamWriter::setBufferSize uses raw malloc at line ~196; "
        "should use std::vector<uint8_t>"
    )


def test_streamreader_size_bound_check_util_010():
    # UTIL-010: StreamReader::runThread at
    ...
    for rf_idx in req_frame_indices:
        ...
        if not has_bound:
            raise AssertionError(
                "UTIL-010: StreamReader reads size as uint32_t and calls "
