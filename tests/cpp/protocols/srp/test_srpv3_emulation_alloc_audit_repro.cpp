 ...
 * ----------------------------------------------------------------------------
 * Description:
 * Regression test guarding against the historical bug in
 * SrpV3Emulation::allocatePage(): the function used to allocate each
 * 4 KiB emulated memory page via raw ``malloc(0x1000)`` and store the
 * raw pointer in the ``memMap_`` std::map.  If the map::insert call
 * threw std::bad_alloc after malloc succeeded, the allocated page was
 * leaked because no RAII owner held it.  The current implementation
 * uses std::make_unique<uint8_t[]>(0x1000) and only releases ownership
 * after a successful insert; this test fails if raw malloc is ever
 * reintroduced.
 * ----------------------------------------------------------------------------
