...
#   SqlLogger._worker swallows DB exceptions silently; callers
...
# ---------------------------------------------------------------------------
# SqlReader uses SQLAlchemy 1.x-only APIs
# ---------------------------------------------------------------------------
def test_sqlreader_uses_sqlalchemy_2_compatible_api():
    """SqlReader uses MetaData(engine) and autoload=True removed in SQLAlchemy 2.0."""
    ...
    assert "MetaData(engine)" not in src_reader, (
        "SqlReader.__init__ calls sqlalchemy.MetaData(engine) "
    ...
    assert "autoload=True" not in src_reader, (
        "SqlReader.__init__ passes autoload=True to sqlalchemy.Table(); "
        "this keyword was removed in SQLAlchemy 2.0"
    )


# ---------------------------------------------------------------------------
# SqlReader.getVariable / getSyslog reference self._conn
# ---------------------------------------------------------------------------
def test_sqlreader_init_sets_conn():
    """SqlReader.__init__ never assigns self._conn; getVariable/getSyslog raise AttributeError."""
    import pyrogue.interfaces._SqlLogging as sql_mod

    init_src = inspect.getsource(sql_mod.SqlReader.__init__)

    assert "self._conn" in init_src, (
        "SqlReader.__init__ does not assign self._conn; "
...
# ---------------------------------------------------------------------------
# SqlLogger._worker swallows DB failures silently
# ---------------------------------------------------------------------------
def test_sqlworker_propagates_db_failure():
    """SqlLogger._worker silently swallows DB failures; no surface to callers."""
    import pyrogue.interfaces._SqlLogging as sql_mod
    ...
    assert exposes_failure, (
        "SqlLogger._worker silently swallows DB failure "
