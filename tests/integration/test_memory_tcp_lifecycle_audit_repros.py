...
# Locate the repository root.  pathlib.Path.parents indexes from 0,
# so parents[0] = tests/integration, parents[1] = tests,
...
def test_memory_tcpclient_uses_unique_ptr_thread():
    ...
    Previously src/rogue/interfaces/memory/TcpClient.cpp line ~120 used::

        this->thread_ = new std::thread(&rim::TcpClient::runThread, this);

    This was replaced with std::unique_ptr<std::thread> for RAII ownership.
...
def test_memory_tcpserver_uses_unique_ptr_thread():
    ...
    Previously src/rogue/interfaces/memory/TcpServer.cpp line ~107 used::
