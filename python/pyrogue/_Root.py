...
class UpdateTracker(object):
    ...
    def _check(self):
        if self._count == 0 or (
            self._period != 0 and (time.time() - self._last) > self._period
        ):
            if len(self._list) != 0:
...
class Root(pr.Device):
        ...
        *,
        ...
        self.add(
            pr.LocalCommand(
                name="SaveState",
                value="",
                function=lambda arg: self.saveYaml(
                    name=arg,
                    readFirst=True,
                    modes=["RW", "RO", "WO"],
                    incGroups=None,
        ...
        self.add(
            pr.LocalCommand(
                name="LoadConfig",
                value="",
                function=lambda arg: self.loadYaml(
                    name=arg,
                    writeEach=False,
                    modes=["RW", "WO"],
                    incGroups=None,
        ...
        self.add(
            pr.LocalCommand(
                name="SetYamlConfig",
                value="",
                function=lambda arg: self.setYaml(
                    yml=arg,
                    writeEach=False,
                    modes=["RW", "WO"],
                    incGroups=None,
        ...
        self.add(
            pr.LocalCommand(
                ...
                value=True,
                retValue="",
                function=lambda arg: self.getYaml(
                    readFirst=arg,
                    modes=["RW", "WO"],
                    incGroups=None,
                    excGroups="NoConfig",
    ...
    def start(self):
        ...
        for i in range(1, len(tmpList)):
            self._log.debug(
                "Comparing {} with address={:#x} to {} with address={:#x} and size={}".format(
                    tmpList[i].path,
                    tmpList[i].address,
                    tmpList[i - 1].path,
                    tmpList[i - 1].address,
            ...
            if (
                (tmpList[i].size != 0)
                and (tmpList[i]._reqSlaveId() == tmpList[i - 1]._reqSlaveId())
                and (
                    tmpList[i].address < (tmpList[i - 1].address + tmpList[i - 1].size)
                )
            ):
                raise pr.NodeError(
                    "{} at address={:#x} overlaps {} at address={:#x} with size={}".format(
                        tmpList[i].path,
                        tmpList[i].address,
                        tmpList[i - 1].path,
                        tmpList[i - 1].address,
        ...
        if self._timeout > 10.0:
            self._log.warning(
                f"Large timeout value of {self._timeout} seconds detected. This may cause unexpected system behavior."
            )
    ...
    def _addVarListenerCpp(self, func, done):
        self.addVarListener(
            lambda path, varValue: func(path, varValue.valueDisp), done=done
        )
    ...
    def saveVariableList(
        self, fname: str, polledOnly: bool = False, incGroups: Optional[List] = None
    ):
        """
        ...
        with open(fname, "w") as f:
            ...
            for v in self.variableList:
                if ((not polledOnly) or (v.pollInterval > 0)) and v.filterByGroup(
                    incGroups=incGroups, excGroups=None
                ):
    ...
    @pr.expose
    def saveYaml(
        self, name, readFirst, modes, incGroups, excGroups, autoPrefix, autoCompress
    ):
    ...
    def loadYaml(
        self, name: Union[str, List[str]], writeEach, modes, incGroups, excGroups
    ):
        ...
        for rl in rawlst:
            ...
            # Entry is a zip file directory
                ...
                with zipfile.ZipFile(base, "r", compression=zipfile.ZIP_LZMA) as myzip:
                    ...
                    if not any(
                        x.startswith("%s/" % sub.rstrip("/")) for x in myzip.namelist()
                    ):
                        raise Exception(
                            "loadYaml: Invalid load file: {}, must be a directory or end in .yml or .yaml".format(
                                rl
                            )
                        )
    ...
    def treeYaml(
        self, modes=["RW", "RO", "WO"], incGroups=None, excGroups=None, properties=None
    ):
    ...
    def _updateWorker(self):
        ...
        while True:
            ...
            # Process list
                ...
                self._log.debug(
                    f"Process update group. Length={len(uvars)}. Entry={list(uvars.keys())[0]}"
                )
                            ...
                            if v.filterByGroup(incGroups, excGroups):
                                try:
                                    ...
                                    if v == self.SystemLog or v == self.SystemLogLast:
                                        print(
                                            "------- Error Executing Syslog Listeners -------"
                                        )
                ...
                self._log.debug(
                    f"Done update group. Length={len(uvars)}. Entry={list(uvars.keys())[0]}"
                )
