...
class Node(object):
    ...
    def _getVariables(self,modes):
        ...
        for key,value in self._nodes.items():
            ...
            elif isinstance(value,pr.Variable) and (value.mode in modes):
    ...
    def _setOrExec(self,d,writeEach,modes):
        ...
        for key, value in d.items():
            ...
            if key in self._nodes:
                ...
                # Execute if command
                elif isinstance(self._nodes[key], pr.Command):
                    self._nodes[key](value)

                # Set value if variable with enabled mode
                elif isinstance(self._nodes[key],pr.Variable) and (self._nodes[key].mode in modes):
