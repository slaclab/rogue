...
class RogueConnection(PyDMConnection):
    ...
    def remove_listener(self, channel: PyDMChannel, destroying: bool) -> None:
        ...
        #
        # IMPORTANT: do NOT tear down the cached VirtualClient here (no
        # client.stop / client.close call).  VirtualClient maintains a
        # process-wide ClientCache keyed by (host, port) (see
        # pyrogue.interfaces._Virtual.VirtualClient.__new__ / ClientCache).
        # The same cached instance is shared by every PyDM widget bound to
        # the same Rogue server, so stopping it on a per-widget
        # remove_listener tears down the REQ/SUB sockets and monitor
        # thread for unrelated live widgets - that was the original
        # PYDM-001 regression.  Cache lifetime is owned by the application
        # (process exit) or by an explicit cache flush, not by individual
        # PyDM channels.
        if self._client is not None:
