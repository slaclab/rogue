...
class Device(pr.Node, rim.Hub):
        ...
        **kwargs,
        ...
        if variable is not None:
            ...
            if recurse:
                for key, value in self.devices.items():
                    value.writeBlocks(
                        force=force, recurse=True, checkEach=checkEach, **kwargs
                    )
    ...
    def verifyBlocks(
        ...
        **kwargs,
        ...
        if variable is not None:
            pr.startTransaction(
                variable._block, type=rim.Verify, checkEach=checkEach, **kwargs
            )  # Verify range is set by previous write

        else:
            for block in self._blocks:
                if block.bulkOpEn:
                    pr.startTransaction(
                        block, type=rim.Verify, checkEach=checkEach, **kwargs
                    )
        ...
        **kwargs,
        ...
        if variable is not None:
            ...
            for block in self._blocks:
                if block.bulkOpEn:
                    pr.startTransaction(
                        block, type=rim.Read, checkEach=checkEach, **kwargs
                    )
    ...
    def writeAndVerifyBlocks(
        ...
        """
        self.writeBlocks(
            force=force, recurse=recurse, variable=variable, checkEach=checkEach
        )
    ...
    def _buildBlocks(self) -> None:
        ...
        for k, n in self.nodes.items():
            ...
            # Align to min access, create list of remote variables
                ...
                self._log.info(
                    f"Before Shift variable {n.name} offset={n.offset} bitSize={n.bitSize} bytes={n.varBytes}"
                )
                ...
                self._log.info(
                    f"Creating variable {n.name} offset={n.offset} bitSize={n.bitSize} bytes={n.varBytes}"
                )
        ...
        for n in remVars:
            if blk is not None and ((blk["offset"] + blk["size"]) > n.offset):
                self._log.info(
                    "Overlap detected var offset={} block offset={} block bytes={}".format(
                        n.offset, blk["offset"], blk["size"]
                    )
                )
                    ...
                    if (n.offset >= b.offset) and ((b.offset + b.size) > n.offset):
                        ...
                        if n.varBytes > b.size:
                            ...
                            raise pr.MemoryError(
                                name=self.path, address=self.address, msg=msg
                            )
        ...
        for b in blocks:
            ...
            if (
                newBlock.size > self._blkMaxAccess()
                or newBlock.size < self._blkMinAccess()
            ):
    ...
    def genDocuments(
        ...
        with open(path + "/" + self.path.replace(".", "_") + ".rst", "w") as file:
            ...
            plist = self.getNodes(
                typ=pr.Process, incGroups=incGroups, excGroups=excGroups
            )
...
class ArrayDevice(Device):
    ...
    def __init__(self, *, arrayClass, number, stride=0, arrayArgs=None, **kwargs):
        ...
        for i in range(number):
            ...
            self.add(arrayClass(name=name, offset=i * stride, **args))
