...
- Added a first-pass `Binding source` link for Boost.Python-generated API
  pages so wrapper `.cpp` sources can be previewed in local Doxygen HTML.
- Fixed the custom Boost.Python directive for Sphinx 8 by reading `docname`
  from `self.state.document.settings.env`.
...
- `docs/src/_ext/rogue_boostpython_api.py`
- `docs/src/_ext/rogue_boostpython_core.py`
...
## Boost.Python Source Link Behavior

We revisited the previously deferred mixed API question and added a first-pass
preview of wrapper-source links for Boost.Python-generated API pages.

Current mechanism:

- `docs/src/_ext/rogue_boostpython_api.py` passes the current Sphinx `docname`
  into the renderer
- `docs/src/_ext/rogue_boostpython_core.py` resolves the wrapper `.cpp`
  file for the binding and maps it to local Doxygen HTML
- Generated Boost.Python API class blocks now include a separate line:
  `Binding source: [SomeClass.cpp]`

Current expected behavior:

- This appears only on pages generated by `.. rogue_boostpython_api:: ...`
- It points to the wrapper/binding implementation `.cpp`, not the underlying
  header/impl pair of the exported C++ class
- The line is intentionally on its own line below the `C++:` line

Example shape:

- `C++: :cpp:class:\`rogue::interfaces::ZmqClient\``
- blank line
- `Binding source: [ZmqClient.cpp]`

Important status:

- This was added as a preview because the user wanted to see how it looks
- The broader design for mixed Python-exported C++ API source linking is still
  not settled
- Do not assume this is final UX

Verification note:

- A Sphinx 8 directive compatibility issue was hit during this work
- The fix was to use `self.state.document.settings.env.docname`, not `self.env`
...
- The user asked to preview the Boost.Python `Binding source` idea, but the
  final mixed-API source-link design is still not settled.
- Treat the current Boost.Python `Binding source` line as provisional UI.
...
6. If Boost.Python source links are questioned, inspect a generated page like
   `docs/build/html/api/python/rogue/interfaces/zmqclient.html` and verify the
   separate `Binding source:` line placement
